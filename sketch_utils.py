...
def plot_batch(inputs, outputs, output_dir, step, use_wandb, title):
    if inputs is None:
        for i, output in enumerate(outputs):
            grid = make_grid(output, normalize=False, pad_value=0)
            npgrid = grid.detach().cpu().numpy()
            im = 255 * np.transpose(npgrid, (1, 2, 0))
            img = Image.fromarray(np.uint8(im))
            img.save("{}/{}_{}".format(output_dir, i, title))
    else:
        for i in range(inputs.shape[0]):
            plt.figure()
            plt.subplot(2, 1, 1)
            grid = make_grid(inputs[i].clone().detach(), normalize=True, pad_value=2)
            ...
            plt.subplot(2, 1, 2)
            grid = make_grid(outputs[i], normalize=False, pad_value=2)
            npgrid = grid.detach().cpu().numpy()
            plt.imshow(np.transpose(npgrid, (1, 2, 0)), interpolation='nearest')
            plt.axis("off")
            plt.title("outputs")
            ...
            plt.savefig("{}/{}_{}".format(output_dir, i, title))
            plt.close()
...
def get_mask_u2net(args, pil_im):
    ...
    model_dir = os.path.join("/home/DreamWire/U2Net_/saved_models/u2net.pth")
    net = U2NET(3, 1)
